function novoElemento(tagName, className, texto) {
...
class Relogio {
  constructor() {
    this.elemento = novoElemento("span", "relogio", "Tempo: 0s");
    this.tempoPassado = 0;
  ...
  iniciarRelogio() {
    this.intervalo = setInterval(() => {
      this.tempoPassado += 1000; // Incrementa o tempo em 1 segundo (1000 ms)
  ...
  atualizarTempoExibido() {
    const segundos = Math.floor(this.tempoPassado / 1000);
    this.elemento.textContent = `Tempo: ${segundos}s`;
  }
}
...
function tocarMusica() {
  document.querySelector("[wm-flappy]").addEventListener("click", function () {
...
class FlappyBird {
  constructor() {
    ...
    areaTotalDoJogo.appendChild(relogio.elemento);
